...
TODO: запросить идентификатор пользователя; если пользователь с таким идентификатором существует в таблице user,
TODO: то вывести на экран двух атлетов: ближайшего по дате рождения к данному пользователю и ближайшего по росту к данному пользователю;
TODO: если пользователя с таким идентификатором нет, вывести соответствующее сообщение."""
""""
Задание 1
...
def request_data():
    ...
    _id = int(input("идентификатор пользователя:"))
    return int(_id)
...
def nearest_by_height(user, session):
    ...
    """
    # athletes_list = session.query(Athelete).all()
    athletes_list = session.query(Athelete).filter(Athelete.height != None).all() # Where Athelete.height != None
